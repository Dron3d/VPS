...
# Это гарантирует создание /run/sshd и запуск на нужном порту даже после ребута
...
# Создаем drop-in конфиг и добавляем порт в основной файл (для двойной надежности)
...
# Очищаем порт перед запуском службы, чтобы не было "Address already in use"
...
# Даем 1 секунду на подъем службы
sleep 1
...
if [ "$REAL_PORT" == "$NEW_SSH_PORT" ]; then
    ...
    # Если ошибка, выведем статус для диагностики
